...
// Deps
var addStats, analyzeUrl, calculateBenchmarkIndex, clearLines, configs, execute, formatRow, formatTime, formatValue, makeTable, program, ucFirst;

import caporalCore from '@caporal/core';

import {
  computeMultiplierMessages
} from './src/cpu-throttling-calculator.js';
...
execute = async function({url, times, devices, blockedUrls, summary}) {
  var benchmarkIndex, bootChrome, chrome, device, i, j, k, len, len1, multiplier, progress, results, theme;
  ...
  for (j = 0, len = devices.length; j < len; j++) {
    device = devices[j];
    results.push((await analyzeUrl({url, times, device, blockedUrls, multiplier, chrome, progress})));
  }
  ...
  console.log(chalk.dim.italic(`benchmarkIndex: ${formatValue(benchmarkIndex, 0)} (cpuSlowdownMultiplier: ${formatValue(multiplier)})`));
  console.log("");
  // Close Chrome
  return chrome.kill();
};
...
calculateBenchmarkIndex = async function({url, chrome, progress}) {
  var flags, report, result;
  flags = {
    onlyCategories: [], // Don't run any audits
    port: chrome.port
  };
  result = (await progress(lighthouse(url, flags), "Benchmarking System", {
    estimate: 10000
  }));
...
analyzeUrl = async function({url, times, device, blockedUrls, multiplier, chrome, progress}) {
  ...
  flags = {
    ...
    throttling: {
      cpuSlowdownMultiplier: multiplier
    },
    port: chrome.port
  };
